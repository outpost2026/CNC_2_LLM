...
from dxf_geometry_indexer_v2 import index_dxf, build_layer_card
...
def test_layer_card_tool_config_mapping(demo_dir, tool_config_path):
    ...
    assert result is not None
    lc = result.get("layer_card", {})
    assert lc.get("total_entities_mapped", 0) >= 0, "total_entities_mapped should be >= 0"


EXPECTED_LAYER_CARD_CSV_COLUMNS = [
    "color_index", "color_name", "entity_count", "total_length_mm",
...
def test_layer_card_csv_columns_match_expected():
    ...
    from dxf_geometry_indexer_v2 import write_layer_card_csv
